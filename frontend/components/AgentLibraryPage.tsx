            ...
            <div className="flex gap-3 mb-6 flex-wrap">
                <div className="relative flex-1 min-w-48">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-aida-teal focus:border-transparent text-sm"
                    />
                </div>
                <div className="flex flex-wrap gap-2">
                    <button
                        onClick={() => setSelectedType(null)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${!selectedType ? 'bg-aida-teal text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
                        All
                    </button>
                    {agentTypes.map(type => (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedType === type ? 'bg-aida-teal text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                        >
                            {type}
                        </button>
                    ))}
                ...
                {Object.entries(grouped).map(([type, agents]) => (
                    <div key={type} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                        <div className="flex items-center gap-3 px-5 py-3 bg-slate-50 border-b border-slate-200">
                            <div className="p-1.5 bg-aida-light rounded-lg">
                                <FolderOpen className="w-4 h-4 text-aida-teal" />
                            </div>
                        ...
                        <div className="divide-y divide-slate-100">
                            {agents.map(agent => (
                                <div key={agent.id} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50 transition-colors group">
                                    ...
                                    <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-aida-teal transition-colors shrink-0" />
                                </div>
                            ))}
                        </div>
                    </div>
