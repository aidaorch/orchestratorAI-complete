...
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({
    ...
    const processImportFile = async (file: File) => {
        try {
            ...
            if (imported) {
                ...
                setImportStatus({ type: 'error', message: 'Invalid template file.' });
            }
        } catch {
    ...
    // ── INLINE MODE (prompt screen) — diagram-only card grid ──────────────────
    if (inline) {
        return (
            <div>
                {filtered.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filtered.map(t => (
                            <div
                                key={t.id}
                                onClick={async () => { await handleLoad(t); }}
    ...
    // ── MODAL / TEMPLATES PAGE MODE ───────────────────────────────────────────
    return (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div
                className={`bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col h-[80vh] ${isDragging ? 'ring-4 ring-emerald-400 ring-inset' : ''}`}
                    ...
                    <div className={`${selected ? 'block' : 'hidden md:flex'} flex-1 overflow-y-auto p-4 sm:p-5`}>
                        {selected ? (
                            <div className="space-y-4">
                                <button onClick={() => setSelected(null)} className="md:hidden flex items-center gap-1.5 text-sm text-aida-teal font-medium mb-2">
                                    ← Back to list
                                </button>
                                {selectedLoading && (
                                ...
                                {/* Preview tab — diagram + prompt only, no step list */}
                                {detailTab === 'preview' && (
                                    <div className="space-y-4">
